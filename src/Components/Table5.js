...
const Head =
  "text-xs text-left text-main font-semibold px-4 py-2 uppercase bg-gray-700 text-gray-200";
const Text =
  "text-xs text-left leading-6 whitespace-nowrap px-4 py-2 text-center";
    ...
    <tr key={i} className="hover:bg-gray-800 transition-colors duration-200">
      {tickets ? (
        <>
          ...
          <td className={`${Text}`}>{data.createAt || "12, Jan 2024"}</td>
          <td className={`${Text}`}>{data.createAt || "12.AM"}</td>
        ...
        <>
          <td className={`${Text} font-bold`}>{data._id || "2R75T8"}</td>
          <td className={`${Text}`}>{data.createAt || "12, Jan 2024"}</td>
    ...
    <div className="overflow-hidden relative w-full rounded-xl border border-gray-700 shadow-md">
      <table className="w-full table-auto divide-y divide-gray-800">
        ...
        <tbody className="bg-gray-900 divide-y divide-gray-800">
          {data.map((item, index) => Rows(item, index, tickets, navigate))}
        </tbody>
      </table>
    </div>
  );
